document.addEventListener("DOMContentLoaded", function () {
    ...
    function displayBlogPost(){
        if(blogPosts && blogPosts.length>0){
            const blog1=blogPosts[0];
            blogContentContainer.innerHTML=`<h3>${blog1.title}</h3><p>${blog1.content}</p>`
        }
        else{
    ...
    function displayComments() {
        displayMessage.innerHTML = comments.map(comment => `<li><strong>${comment.name}:</strong> ${comment.message}</li>`).join('');
    }
    ...
    submitButton.addEventListener('click', function () {
        ...
        if (newMessage && commenterName) {
            ...
            comments.push({ name: commenterName, message: newMessage });
